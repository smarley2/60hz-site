...
Expected result: FAIL because the two new PNG files do not exist yet.
...
- [ ] **Step 5: Run the focused test and inspect asset metadata**
...
Expected result: the test passes and both files are valid PNG images.
...
Expected result: PASS; the existing copy, email, viewport, palette, breakpoint, and no-CNAME checks remain valid.
...
- Produces: a pushed GitHub branch and draft PR targeting the repository default branch, unless direct main-branch publication is explicitly selected by the user.
